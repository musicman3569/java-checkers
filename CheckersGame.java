   ...
   implements MouseListener
{
   ...
   public void loadImages() throws IOException {
      img_redPiece = getImage("/images/red.png");
      img_redKing = getImage("/images/redKing.png");
      img_blackPiece = getImage("/images/black.png");
      img_blackKing = getImage("/images/blackKing.png");
      img_background = getImage("/images/background.png");
      img_board = getImage("/images/board.png");
   }
   ...
   {
      //
      // "New Game" button.  Asks for the player's name if it's the first
      ...
      btnNewGame.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent evt)
         {
            ...
            if (playerName == null)
            {
               changeStatus("Waiting for user's name to be entered...");
               String title = "Create New Game";
               String message = "Please enter your name:";
               playerName = JOptionPane.showInputDialog(null, message, title, 1);
            }
            ...
            changeStatus("Waiting for user to select difficulty...");
            currGame.newGame();
            
            changeStatus("Creating new game. Please wait . . .");
            rootPanel.paintImmediately(0, 0, 600, 600);
            gameInProgress = true;
            legalMoves = currGame.getLegalMoves(GameBoard.PLAYER);
            selectedRow = -1;    // indicate that no piece is selected yet
            middlePanel.getPiecePositions();
            rootPanel.paintImmediately(0, 0, 600, 600);
            middlePanel.setGameSpeed();
            btnComputerFirst.setVisible(true);
            changeStatus(
               "Select a piece to move   OR   'Let Computer Move First'");
         }
      });
      
      //
      // "Highlight Moves ON/OFF" button.  Toggles whether legal moves
      // and move locations are automatically highlighted on screen.
      //
      btnShowHighlight.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent evt)
         {
            ...
            if (showHighlight)
            {
               btnShowHighlight.setText("Highlight Moves ON ");
            }
            else
            {
               btnShowHighlight.setText("Highlight Moves OFF");
            }
            rootPanel.paintImmediately(0, 0, 600, 600);
         }
      });
      ...
      btnShowRules.addActionListener(new ActionListener()
      {
         public void actionPerformed(ActionEvent evt)
         {
            ...
            try
            {
               tp.setPage(new URL(basePath + "/images/checkersRules.html"));
            }
            catch (Exception e)
            {
               tp.setText("Error Loading images/checkersRules.html");
               e.printStackTrace();
            }
            rootPanel.paintImmediately(0, 0, 600, 600);
         }
